...
while (iter.hasNext())
{
 ...
 上面的便利可以用更简练的for-each，for-each就是用itrator来实现的。编译器简单地将“ foreach ” 循环翻译为带有迭代器的循环。
...
实现这个接口，实例的对象就会允许使用  foreach语句遍历（说明foreach是通过迭代器实现的）

3.forEachRemaining

```java
 iterator.forEachRemaining(el ement - > do something with element);
```
